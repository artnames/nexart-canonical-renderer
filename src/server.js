...
function validateCodeSafety(code) {
  ...
  if (code.includes("[") && code.includes("]")) {
    const bracketContent = code.match(/\[\s*["'`].*["'`]\s*\]/g) || [];
    for (const bracket of bracketContent) {
      if (/process|global|require|import|eval|constructor|prototype|__proto__|Proxy|Reflect/.test(bracket)) {
        throw new Error("Forbidden dynamic property access detected");
      }
    }
  }
}
...
function executeCodeMode(code, config) {
  ...
  const wrappedSetup = new Function(
    ...
    "tGlobal",
    "safeGlobals",
    `with(p) { with(safeGlobals) { ${setupCode} } }`
  );

  wrappedSetup(p, p.VAR, 0, 0, 0, 0, safeGlobals);
...
app.post("/api/v1/render", (req, res) => {
  ...
  try {
    ...
    if (vars.length > 10) {
      return res.status(400).json({
        error: "INVALID_VARS",
        message: "VAR array must have at most 10 elements",
      });
    }

    for (let i = 0; i < vars.length; i++) {
      if (typeof vars[i] !== "number" || vars[i] < 0 || vars[i] > 100) {
        return res.status(400).json({
          error: "INVALID_VAR_VALUE",
          message: `VAR[${i}] must be a number between 0 and 100`,
        });
      }
    }
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        ...
        message: "Snapshot does not match expected shape",
      });
    }
    ...
    const width = snapshot.width ?? 256;
    const height = snapshot.height ?? 256;
    const seed = snapshot.seed ?? "0";
    ...
    const code = snapshot.code || `
      function setup() {
        const v0 = VAR[0] / 100;
        const v1 = VAR[1] / 100;
        background(20 + random(40));
        const size = 40 + v0 * 160;
        const x = random(width - size);
        const y = random(height - size);
        fill(100 + random(155), 100 + random(155), 100 + random(155));
        rect(x, y, size, size);
      }
    `;

    const canvas = executeCodeMode(code, { width, height, seed, vars });
...
app.post("/api/v1/hash", (req, res) => {
  try {
    ...
    return res.status(400).json({
      ...
      message:
        "Must provide type (snapshot|image|code) and corresponding data",
    });
  } catch (error) {
...
app.post("/api/v1/verify", (req, res) => {
  ...
  try {
    ...
    if (hash_type === "snapshot") {
      computedHash = computeSnapshotHash({
        code: snapshot.code,
        seed,
        vars,
        width,
